...
export class LessonsEffects {
  @Effect() loadLessons$ = this.actions$.pipe(
    ...
    fetch({
      run: (action) => this.lessonsService.all().pipe(
        map((lessons: Lesson[]) => LessonsActions.loadLessonsSuccess({ lessons }))
      ),
  ...
  @Effect() loadLesson$ = this.actions$.pipe(
    ...
    fetch({
      run: (action) => this.lessonsService.find(action.lessonId).pipe(
        map((lesson: Lesson) => LessonsActions.loadLessonSuccess({ lesson }))
      ),
  ...
  @Effect() createLesson$ = this.actions$.pipe(
    ...
    pessimisticUpdate({
      run: (action) => this.lessonsService.create(action.lesson).pipe(
        map((lesson: Lesson) => LessonsActions.createLessonSuccess({ lesson }))
      ),
  ...
  @Effect() updateLesson$ = this.actions$.pipe(
    ...
    pessimisticUpdate({
      run: (action) => this.lessonsService.update(action.lesson).pipe(
        map((lesson: Lesson) => LessonsActions.updateLessonSuccess({ lesson }))
      ),
  ...
  @Effect() deleteLesson$ = this.actions$.pipe(
    ...
    pessimisticUpdate({
      run: (action) => this.lessonsService.delete(action.lesson.id).pipe(
        map((lesson: Lesson) => LessonsActions.deleteLessonSuccess({ lesson })),
      ),
