...
export class CoursesEffects {
  @Effect() loadCourses$ = this.actions$.pipe(
    ...
    fetch({
      run: (action) => this.coursesService.all().pipe(
  ...
  @Effect() loadCourse$ = this.actions$.pipe(
    ...
    fetch({
      run: (action) => this.coursesService.find(action.courseId).pipe(
        map((course: Course) => CoursesActions.loadCourseSuccess({ course }))
      ),
  ...
  @Effect() createCourse$ = this.actions$.pipe(
    ...
    pessimisticUpdate({
      run: (action) => this.coursesService.create(action.course).pipe(
  ...
  @Effect() updateCourse$ = this.actions$.pipe(
    ...
    pessimisticUpdate({
      run: (action) => this.coursesService.update(action.course).pipe(
  ...
  @Effect() deleteCourse$ = this.actions$.pipe(
    ...
    pessimisticUpdate({
      run: (action) => this.coursesService.delete(action.course.id).pipe(
        map((course: Course) => CoursesActions.deleteCourseSuccess({ course })),
      ),
