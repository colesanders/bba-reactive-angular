...
export class UsersEffects {
  @Effect() loadUsers$ = this.actions$.pipe(
    ...
    fetch({
      run: (action) => this.usersService.all().pipe(
  ...
  @Effect() loadUser$ = this.actions$.pipe(
    ...
    fetch({
      run: (action) => this.usersService.find(action.userId).pipe(
  ...
  @Effect() createUser$ = this.actions$.pipe(
    ...
    pessimisticUpdate({
      run: (action) => this.usersService.create(action.user).pipe(
  ...
  @Effect() updateUser$ = this.actions$.pipe(
    ...
    pessimisticUpdate({
      run: (action) => this.usersService.update(action.user).pipe(
  ...
  @Effect() deleteUser$ = this.actions$.pipe(
    ...
    pessimisticUpdate({
      run: (action) => this.usersService.delete(action.user.id).pipe(
